...
function generateVenues(count) {
    ...
    for (var i = 1; i <= count; i++) {
        ...
        var venueDetails = __assign(__assign({}, venue), { venueId: venue.id, numberOfReviews: faker_1.faker.number.int({ min: 10, max: 500 }), description: faker_1.faker.lorem.paragraph(), features: faker_1.faker.helpers.arrayElements(featuresList, faker_1.faker.number.int({ min: 5, max: 10 })), sleepingDetails: {
                maxCapacity: capacity + faker_1.faker.number.int({ min: 2, max: 4 }),
                amountOfBeds: faker_1.faker.number.int({ min: 2, max: 6 }),
            }, checkInHour: "12pm", checkOutHour: "10am", distanceFromCityCenterInKM: faker_1.faker.number.int({ min: 1, max: 15 }), contactDetails: {
